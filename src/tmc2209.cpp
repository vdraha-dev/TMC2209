...
TMC2209::TMC2209() {
	m_transport = nullptr;
	m_serial_address = SERIAL_ADDRESS_0;
	m_hardware_enable_pin = GPIO_NUM_NC;
	m_cool_step_enabled = false;
}


void TMC2209::setup(
	transport::interface::Transport & transport,
...
void TMC2209::setHardwareEnablePin(gpio_num_t hardware_enable_pin) {
	m_hardware_enable_pin = hardware_enable_pin;
	// pinMode(m_hardware_enable_pin, OUTPUT);
	// digitalWrite(m_hardware_enable_pin, HIGH);

	gpio_set_direction(m_hardware_enable_pin, GPIO_MODE_OUTPUT);
	gpio_set_level(m_hardware_enable_pin, 1);
}

void TMC2209::enable() {
	if (m_hardware_enable_pin >= 0) {
		// digitalWrite(m_hardware_enable_pin, LOW);
		gpio_set_level(m_hardware_enable_pin, 0);
	}
	m_chopper_config.toff = m_toff;
...
void TMC2209::disable() {
	...
	m_chopper_config.toff = TOFF_DISABLE;
	writeStoredChopperConfig();
}
...
void TMC2209::setMicrostepsPerStepPowerOfTwo(uint8_t exponent) {
	switch (exponent) {
		case 0: {
			m_chopper_config.mres = MRES_001;
			break;
		}
		case 1: {
			m_chopper_config.mres = MRES_002;
			break;
		}
		case 2: {
			m_chopper_config.mres = MRES_004;
			break;
		}
		case 3: {
			m_chopper_config.mres = MRES_008;
			break;
		}
		case 4: {
			m_chopper_config.mres = MRES_016;
			break;
		}
		case 5: {
			m_chopper_config.mres = MRES_032;
			break;
		}
		case 6: {
			m_chopper_config.mres = MRES_064;
			break;
		}
		case 7: {
			m_chopper_config.mres = MRES_128;
			break;
		}
		case 8:
		default: {
			m_chopper_config.mres = MRES_256;
			break;
		}
	}
	writeStoredChopperConfig();
...
void TMC2209::setRunCurrent(uint8_t percent) {
	...
	m_driver_current.irun = run_current;
...
void TMC2209::setHoldCurrent(uint8_t percent) {
	...
	m_driver_current.ihold = hold_current;
	writeStoredDriverCurrent();
}
...
void TMC2209::setHoldDelay(uint8_t percent) {
	...
	m_driver_current.iholddelay = hold_delay;
	writeStoredDriverCurrent();
}

void TMC2209::setAllCurrentValues(
...
void TMC2209::setRMSCurrent(uint16_t mA, float rSense, float holdMultiplier) {
	...
	m_driver_current.irun = CS;
	m_driver_current.ihold = CS*holdMultiplier;
	writeStoredDriverCurrent();
}

void TMC2209::enableDoubleEdge() {
	m_chopper_config.double_edge = DOUBLE_EDGE_ENABLE;
	writeStoredChopperConfig();
}

void TMC2209::disableDoubleEdge() {
	m_chopper_config.double_edge = DOUBLE_EDGE_DISABLE;
	writeStoredChopperConfig();
}

void TMC2209::enableVSense() {
	m_chopper_config.vsense = VSENSE_ENABLE;
	writeStoredChopperConfig();
}

void TMC2209::disableVSense() {
	m_chopper_config.vsense = VSENSE_DISABLE;
	writeStoredChopperConfig();
}

void TMC2209::enableInverseMotorDirection() {
	m_global_config.shaft = 1;
	writeStoredGlobalConfig();
}

void TMC2209::disableInverseMotorDirection() {
	m_global_config.shaft = 0;
	writeStoredGlobalConfig();
}

void TMC2209::setStandstillMode(TMC2209::StandstillMode mode) {
	m_pwm_config.freewheel = mode;
	writeStoredPwmConfig();
}

void TMC2209::enableAutomaticCurrentScaling() {
	m_pwm_config.pwm_autoscale = STEPPER_DRIVER_FEATURE_ON;
	writeStoredPwmConfig();
}

void TMC2209::disableAutomaticCurrentScaling() {
	m_pwm_config.pwm_autoscale = STEPPER_DRIVER_FEATURE_OFF;
	writeStoredPwmConfig();
}

void TMC2209::enableAutomaticGradientAdaptation() {
	m_pwm_config.pwm_autograd = STEPPER_DRIVER_FEATURE_ON;
	writeStoredPwmConfig();
}

void TMC2209::disableAutomaticGradientAdaptation() {
	m_pwm_config.pwm_autograd = STEPPER_DRIVER_FEATURE_OFF;
	writeStoredPwmConfig();
}

void TMC2209::setPwmOffset(uint8_t pwm_amplitude) {
	m_pwm_config.pwm_offset = pwm_amplitude;
	writeStoredPwmConfig();
}

void TMC2209::setPwmGradient(uint8_t pwm_amplitude) {
	m_pwm_config.pwm_grad = pwm_amplitude;
	writeStoredPwmConfig();
}
...
void TMC2209::enableStealthChop() {
	m_global_config.enable_spread_cycle = 0;
	writeStoredGlobalConfig();
}

void TMC2209::disableStealthChop() {
		m_global_config.enable_spread_cycle = 1;
		writeStoredGlobalConfig();
}
...
TMC2209::Settings TMC2209::getSettings()
{
	...
	if (settings.is_communicating) {
		...
		settings.is_setup = m_global_config.pdn_disable;
		settings.software_enabled = (m_chopper_config.toff > TOFF_DISABLE);
		settings.microsteps_per_step = getMicrostepsPerStep();
		settings.inverse_motor_direction_enabled = m_global_config.shaft;
		settings.stealth_chop_enabled = not m_global_config.enable_spread_cycle;
		settings.standstill_mode = m_pwm_config.freewheel;
		settings.irun_percent = currentSettingToPercent(m_driver_current.irun);
		settings.irun_register_value = m_driver_current.irun;
		settings.ihold_percent = currentSettingToPercent(m_driver_current.ihold);
		settings.ihold_register_value = m_driver_current.ihold;
		settings.iholddelay_percent = holdDelaySettingToPercent(m_driver_current.iholddelay);
		settings.iholddelay_register_value = m_driver_current.iholddelay;
		settings.automatic_current_scaling_enabled = m_pwm_config.pwm_autoscale;
		settings.automatic_gradient_adaptation_enabled = m_pwm_config.pwm_autograd;
		settings.pwm_offset = m_pwm_config.pwm_offset;
		settings.pwm_gradient = m_pwm_config.pwm_grad;
		settings.cool_step_enabled = m_cool_step_enabled;
		settings.analog_current_scaling_enabled = m_global_config.i_scale_analog;
		settings.internal_sense_resistors_enabled = m_global_config.internal_rsense;
	} else {
...
{
  	m_serial_address = serial_address;

	m_global_config.bytes = 0;
	m_global_config.i_scale_analog = 0;
	m_global_config.pdn_disable = 1;
	m_global_config.mstep_reg_select = 1;
	m_global_config.multistep_filt = 1;

	writeStoredGlobalConfig();
}

void TMC2209::setRegistersToDefaults() {
	m_driver_current.bytes = 0;
	m_driver_current.ihold = IHOLD_DEFAULT;
	m_driver_current.irun = IRUN_DEFAULT;
	m_driver_current.iholddelay = IHOLDDELAY_DEFAULT;
	write(ADDRESS_IHOLD_IRUN, m_driver_current.bytes);

	m_chopper_config.bytes = CHOPPER_CONFIG_DEFAULT;
	m_chopper_config.tbl = TBL_DEFAULT;
	m_chopper_config.hend = HEND_DEFAULT;
	m_chopper_config.hstart = HSTART_DEFAULT;
	m_chopper_config.toff = TOFF_DEFAULT;
	write(ADDRESS_CHOPCONF, m_chopper_config.bytes);

	m_pwm_config.bytes = PWM_CONFIG_DEFAULT;
	write(ADDRESS_PWMCONF, m_pwm_config.bytes);

	m_cool_config.bytes = COOLCONF_DEFAULT;
	write(ADDRESS_COOLCONF, m_cool_config.bytes);
...
void TMC2209::readAndStoreRegisters() {
	m_global_config.bytes = readGlobalConfigBytes();
	m_chopper_config.bytes = readChopperConfigBytes();
	m_pwm_config.bytes = readPwmConfigBytes();
}
...
void TMC2209::minimizeMotorCurrent() {
	m_driver_current.irun = CURRENT_SETTING_MIN;
	m_driver_current.ihold = CURRENT_SETTING_MIN;
	writeStoredDriverCurrent();
}
...
void TMC2209::write(uint8_t register_address, uint32_t data) {
	...
	write_datagram.serial_address = m_serial_address;
...
uint32_t TMC2209::read(uint8_t register_address)
{
	...
	read_request_datagram.serial_address = m_serial_address;
...
uint8_t TMC2209::percentToCurrentSetting(uint8_t percent) {
  	return map(
		constrain_(percent, PERCENT_MIN, PERCENT_MAX), // constrained_percent
		PERCENT_MIN,
		PERCENT_MAX,
		CURRENT_SETTING_MIN,
		CURRENT_SETTING_MAX
...
void TMC2209::writeStoredGlobalConfig() {
  	write(ADDRESS_GCONF, m_global_config.bytes);
}
...
void TMC2209::writeStoredDriverCurrent() {
	...
	if (m_driver_current.irun >= SEIMIN_UPPER_CURRENT_LIMIT) {
		m_cool_config.seimin = SEIMIN_UPPER_SETTING;
	}
	else {
		m_cool_config.seimin = SEIMIN_LOWER_SETTING;
	}
	if (m_cool_step_enabled) {
		write(ADDRESS_COOLCONF, m_cool_config.bytes);
	}
}
